...
def run_comparison(
    ...
    merged = pd.merge(
        grp1, grp2, on=["SHARE_CODE", "INVESTOR_NAME"], how="outer", suffixes=("_prev", "_curr")
    )
    ...
    for idx, r in remaining_exited.iterrows():
        ...
        exits_by_code_shares.setdefault((code, shares), []).append((idx, str(r["INVESTOR_NAME"]), float(r["PERCENTAGE_prev"])))
    ...
    for idx, r in remaining_entered.iterrows():
        ...
        candidates = exits_by_code_shares.get((code, shares), [])
        best_cand = None
        for c_idx, c_name, c_pct in candidates:
            if c_idx not in matched_idx_prev:
                # Check for token overlap (at least 1 word overlaps)
                c_tokens = set(get_tokens(c_name))
                overlap = curr_tokens & c_tokens
                if overlap:
                    best_cand = (c_idx, c_name, c_pct)
                    break

        if best_cand:
            c_idx, c_name, c_pct = best_cand
            matched_idx_prev.add(c_idx)
            matched_idx_curr.add(idx)
            name_changes.append(
                {
